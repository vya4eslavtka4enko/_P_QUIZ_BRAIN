class QuizBrain:
    def __init__(self, q_list):
        self.question_number = 0
        self.question_list = q_list
        self.score = 0
    def still_has_questions(self):
        return self.question_number < len(self.question_list)
    def next_question(self):
        # retriev the item at the current question_number from the question list
        # use the input() function to show the user the question text and ask for the user answer
        current_question = self.question_list[self.question_number]
        self.question_number += 1
        user_answer = input(f"Q:{self.question_number+1}: {current_question.text} (True/False)")
        self.check_answer(user_answer, current_question.answer, self.question_number)
    def check_answer(self,user_answer, correct_answer,current_question_number):
        if user_answer.lower() == correct_answer.lower():
            print("You are got it right")
            self.score += 1
        else:
            print("That's wrong")
        print(f"Te correct answer was : {correct_answer}")
        print(f"Your current score is {self.score}/ {current_question_number}")

